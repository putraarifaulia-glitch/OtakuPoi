...
<div class="min-h-screen bg-gray-50 dark:bg-dark-bg py-12 transition-colors duration-300">
    <div class="container mx-auto px-4 max-w-3xl">
        <div class="mb-8 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Edit News Article</h1>
                <p class="text-gray-500 dark:text-gray-400">Refine and update your content.</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-3xl shadow-sm border border-gray-100 dark:border-dark-border p-8 transition-colors">
            <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 uppercase tracking-wide px-1">Article Image</label>
                    <div class="flex flex-col md:flex-row gap-6 items-start">
                        <div class="flex-shrink-0">
                            <p class="text-xs text-gray-400 dark:text-gray-500 mb-2 font-bold uppercase tracking-wider">Current/New Image:</p>
                            <div id="image-preview">
                                @if($news->image_path)
                                    <img src="{{ asset('storage/' . $news->image_path) }}" class="w-40 h-24 object-cover rounded-xl shadow-sm border border-gray-100 dark:border-dark-border">
                                @else
                                    <div class="w-40 h-24 bg-gray-100 dark:bg-dark-bg rounded-xl flex items-center justify-center text-gray-400 dark:text-gray-500 border border-gray-100 dark:border-dark-border">No Image</div>
                                @endif
                            </div>
                        </div>
                        <div class="flex-grow w-full">
                            <p class="text-xs text-gray-400 dark:text-gray-500 mb-2 font-bold uppercase tracking-wider">Upload New Image (optional):</p>
                            <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-200 dark:border-dark-border border-dashed rounded-xl hover:border-indigo-600 dark:hover:border-indigo-500 transition-colors bg-gray-50/30">
                                <div class="space-y-1 text-center">
                                    <svg class="mx-auto h-8 w-8 text-gray-400 dark:text-gray-600" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                ...
                <div>
                    ...
                    <textarea name="content" rows="12" required 
                        ...
                        placeholder="Write the article body here...">{{ old('content', $news->content) }}</textarea>
    ...
    function previewImage(event) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if (!img) {
                    preview.innerHTML = '<img src="" class="w-40 h-24 object-cover rounded-xl shadow-sm border border-gray-100 dark:border-dark-border">';
