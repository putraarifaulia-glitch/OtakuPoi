    ...
    <div class="container mx-auto px-4 max-w-3xl">
        <div class="mb-8 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-500">Update article details and content.</p>
            </div>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" required
                        class="w-full px-4 py-3 rounded-xl border-gray-200 focus:border-indigo-600 focus:ring-indigo-600 transition-all bg-gray-50/50">{{ old('content', $news->content) }}</textarea>
                    @error('content') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Article Image</label>
                    <div class="flex flex-col md:flex-row gap-6 items-start">
                        @if($news->image_path)
                            <div class="flex-shrink-0">
                                <p class="text-xs text-gray-400 mb-2">Current Image:</p>
                                <img src="{{ asset('storage/' . $news->image_path) }}" class="w-40 h-24 object-cover rounded-xl shadow-sm border border-gray-100">
                            </div>
                        @endif
                        <div class="flex-grow w-full">
                            <p class="text-xs text-gray-400 mb-2">Upload New Image (optional):</p>
                            <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-200 border-dashed rounded-xl hover:border-indigo-600 transition-colors bg-gray-50/30">
