    ...
    <div class="container mx-auto px-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <p class="text-gray-500">Create, edit, and manage news articles for OtakuPoi.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            <th class="px-6 py-4 text-sm font-bold text-gray-600 uppercase tracking-wider">Thumbnail</th>
                            <th class="px-6 py-4 text-sm font-bold text-gray-600 uppercase tracking-wider">Title & Date</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-6 py-4">
                                @if($item->image_path)
                                    <img src="{{ asset('storage/' . $item->image_path) }}" class="w-20 h-12 object-cover rounded-lg shadow-sm border border-gray-100">
                                @else
                                    <div class="w-20 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500 mt-1">{{ $item->created_at->format('M d, Y • H:i') }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('news.show', $item->slug) }}" target="_blank" class="p-2 text-gray-400 hover:text-indigo-600 transition-colors" title="View Article">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.news.edit', $item->id) }}" class="p-2 text-gray-400 hover:text-amber-500 transition-colors" title="Edit Article">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <form action="{{ route('admin.news.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this news article?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 text-gray-400 hover:text-red-500 transition-colors" title="Delete Article">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if($news->isEmpty())
                <div class="py-20 text-center">
                    ...
                    <p class="text-gray-400 font-medium text-lg">No news articles found.</p>
                </div>
            @endif
        </div>
    </div>
